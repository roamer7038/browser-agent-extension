...
graph TD
    ...
    subgraph "External Providers"
        ...
        WebAPI((Web APIs / Integrations)):::external
    end
...
### ウェブサービスインテグレーション (lib/services/integrations)

MCPを提供していないウェブサービス（Ollamaの内部検索など）と連携するため、固定スキーマのWeb APIを直接叩くLangChainツールを提供する層です。各サービスごとにAPI認証を管理し、ToolFactoryから動的にエージェントに機能（ツール）を注入します。
