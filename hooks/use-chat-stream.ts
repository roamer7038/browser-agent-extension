...
export function useChatStream() {
  ...
  const setupStreamListener = (port: chrome.runtime.Port, baseMessages: Message[]) => {
    ...
    port.onDisconnect.addListener(() => {
      // We use state lookup in the component if needed, or simply let the store manage it
      setActivePort(null);
      setIsLoading(false);
    });
